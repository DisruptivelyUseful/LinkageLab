...
- **Camera view** — **Capture** saves the current camera and fold angle, **Auto-frame** frames the step's parts, **Go to** moves the camera there, **Clear** falls back to auto-framing at playback time. Auto gives hardware steps (fit brackets, every fasten) a *close-up* view (`view.frame = 'targets'`). With **Full Detail** hardware on, playback switches to the **parts detail view** of the joint (the same in-structure close-up the Hardware Detail editor uses, with the assembly seated rather than exploded) and, for fasten steps, moves from joint to joint as each one is bolted. Without detailed hardware the camera frames the step's parts at the step's fold angle, looking inward from outside the ring (`radial`). Parts of other modules are faded during close-ups. Beam and module steps keep the wide view.
- **Transition / Duration** — camera tween time and operation time in milliseconds.
- Kind-specific fields: stock length and kerf (cut), bit diameter (drill), approach direction, travel and *One at a time* (place: parts arrive in sequence instead of together, used for the solar panels), turns, *All modules* and *One joint at a time* (fasten: joints are bolted in sequence and the camera follows each one; Auto turns it on).
